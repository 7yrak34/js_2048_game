...
const game = new (0, _gameClass.Game)();
...
button.addEventListener("click", ()=>{
    ...
    if (button.classList.contains("start")) {
        ...
        document.addEventListener("keydown", (key)=>{
            if (game.status === (0, _gameClass.Game).STATUS_PLAYING) {
                switch(key.key){
                    case "ArrowLeft":
                        game.moveLeft();
                        break;
...
function displayOnField(matrix) {
    for(let rowIndex = 0; rowIndex < matrix.length; rowIndex++)for(let cellIndex = 0; cellIndex < matrix[rowIndex].length; cellIndex++){
        ...
        const cell = cells[(0, _gameClass.Game).GRID_SIZE * rowIndex + cellIndex];
    ...
    switch(game.status){
        case (0, _gameClass.Game).STATUS_WIN:
            messageWin.classList.remove("hidden");
            break;
        case (0, _gameClass.Game).STATUS_LOSE:
            messageLose.classList.remove("hidden");
            break;
    }
}

},{"../modules/Game.class":"eIbh2"}],"eIbh2":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "Game", ()=>Game);
var _getRandomNumberJs = require("../helpers/getRandomNumber.js");
class Game {
    /**
   ...
   */ constructor(initialState = Array.from({
    ...
    updateState(newState, newScore) {
        ...
        if (this.state.some((row)=>row.some((cell)=>cell === Game.WIN_NUMBER))) {
    ...
    rotateMatrix(matrix) {
        ...
        for(let cellIndex = 0; cellIndex < Game.GRID_SIZE; cellIndex++){
            const column = [];
            for(let rowIndex = 0; rowIndex < Game.GRID_SIZE; rowIndex++)column.push(matrix[rowIndex][cellIndex]);
   ...
   */ getStatus() {
    ...
    addRandomCell() {
        ...
        const randomIndex = (0, _getRandomNumberJs.getRandomNumber)(0, availableCells.length - 1);
...
},{"../helpers/getRandomNumber.js":"6QsYe","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"6QsYe":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
